...
/**
 * Clone a simulator device with new device name from given device name (xcode >= 10 only)
 *
 * @param {string} deviceName - The device name clone from
 * @param {string} newDeviceName - The device name clone to
 * @param {Array<string>} destinationDeviceSet - A set of destinations
 * @return {string} The UDID of the newly created device.
 * @throws {Error} If the corresponding simctl subcommand command returns non-zero return code.
 */
async function cloneDevice (deviceName, newDeviceName, destinationDeviceSet = []) {
  let udid;

  log.debug(`Cloning simulator with newDeviceName '${newDeviceName}' from deviceName '${deviceName}'`);
  try {
    let out;
    if (_.isArray(destinationDeviceSet) && !_.isEmpty(destinationDeviceSet)) {
      out = await simExec('clone', 0, [deviceName, newDeviceName, destinationDeviceSet]);
    } else {
      out = await simExec('clone', 0, [deviceName, newDeviceName]);
    }
    udid = out.stdout.trim();
  } catch (err) {
    let reason = err.message;
    ...
    log.errorAndThrow(`Could not clone simulator with newDeviceName '${newDeviceName}' from deviceName '${deviceName}'.` +
        `Reason: '${reason}'`);
  }
...
export { installApp, removeApp, launch, spawn, spawnSubProcess, openUrl,
         terminate, shutdown, createDevice, cloneDevice, getAppContainer, getScreenshot,
